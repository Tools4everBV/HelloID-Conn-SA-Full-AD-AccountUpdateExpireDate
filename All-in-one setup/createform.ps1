...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD Account - Update account expires" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-calendar" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@; 
	...
	$tmpVariables = @'
[{"name":"blnexpdate","value":"{{form.blnExpDate}}","secret":false,"typeConstraint":"string"},{"name":"expiredate","value":"{{form.expireDate}}","secret":false,"typeConstraint":"string"},{"name":"userPrincipalName","value":"{{form.gridUsers.UserPrincipalName}}","secret":false,"typeConstraint":"string"}]
'@ 

	$delegatedFormTaskGuid = [PSCustomObject]@{} 
	Invoke-HelloIDAutomationTask -TaskName "AD-user-set-expiredate" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD Account - Update account expires' already exists. Nothing to do with the Delegated Form task..." 
}
